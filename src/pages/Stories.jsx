          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Select value={selectedCategory} onValueChange={setSelectedCategory}>
              {({ isOpen, setIsOpen, value, onValueChange }) => (
                <>
                  <SelectTrigger isOpen={isOpen} setIsOpen={setIsOpen}>
                    <SelectValue placeholder="Category" value={value}>
                      {value === "all" ? "All Categories" : value}
                    </SelectValue>
                  </SelectTrigger>
                  <SelectContent isOpen={isOpen}>
                    <SelectItem value="all" onSelect={() => {onValueChange("all"); setIsOpen(false);}}>
                      All Categories
                    </SelectItem>
                    {categories.map((category) => (
                      <SelectItem key={category} value={category} onSelect={() => {onValueChange(category); setIsOpen(false);}}>
                        {category}
                      </SelectItem>
                    ))}
                  </SelectContent>
            ...
            <Select value={selectedAgeGroup} onValueChange={setSelectedAgeGroup}>
              {({ isOpen, setIsOpen, value, onValueChange }) => (
                <>
                  <SelectTrigger isOpen={isOpen} setIsOpen={setIsOpen}>
                    <SelectValue placeholder="Age Group" value={value}>
                  ...
                  <SelectContent isOpen={isOpen}>
                    ...
                    {ageGroups.map((ageGroup) => (
                      <SelectItem key={ageGroup} value={ageGroup} onSelect={() => {onValueChange(ageGroup); setIsOpen(false);}}>
                        {ageGroup}
                      </SelectItem>
                    ))}
                  </SelectContent>
